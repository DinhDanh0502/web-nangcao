            ...
            <div class="row">
                ...
                <div class="col-md-2">
                    <select name="search_by" class="form-control">
                        ...
                        <option value="{{ $value }}" {{ ($searchBy ?? 'all') === $value ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-2">
                    <select name="sort_by" class="form-control">
                        ...
                        <option value="{{ $value }}" {{ ($sortBy ?? 'id_desc') === $value ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                        @endforeach
                    </select>
            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $artist->name }}</td>
                        <td>{{ Str::limit($artist->bio ?? 'Chưa có tiểu sử', 100) }}</td>
                        <td>{{ $artist->songs_count ?? 0 }}</td>
                        <td>
                            <a href="{{ route('admin.artists.show', $artist->id) }}" class="btn btn-sm btn-info">
